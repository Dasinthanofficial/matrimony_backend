// ===== FILE: ./controllers/profileController.js (UPDATED) =====
...
const calculateCompletion = (profile) => {
  ...
  details.physicalAttributes = Math.round(
    (hasHeight ? 50 : 0) + (physicalOptionalFilled / Math.max(physicalOptional.length, 1)) * 50
  );
  ...
  details.lifestyle = Math.round(
    (lifestyleFilled / Math.max(lifestyleFields.length, 1)) * 70 + hobbiesFilled * 30
  );
  ...
  const careerOptional = ['educationField', 'institution', 'company', 'jobTitle', 'monthlyIncome', 'annualIncome'];
  ...
  details.education =
    careerFilled > 0 ? Math.round((careerFilled / Math.max(careerFields.length, 1)) * 100) : 0;
...
export const getMyProfile = async (req, res) => {
  try {
    ...
    const profile = await Profile.findOne({ userId }).populate(
      'userId',
      'email phone countryCode isEmailVerified isPhoneVerified isPremium'
    );
...
export const createProfile = async (req, res) => {
  try {
    ...
    if (!normalizedCountry) errors.push({ field: 'country', message: 'Country is required' });
...
export const getProfileById = async (req, res) => {
  try {
    ...
    // Agency profiles are NEVER locked — fully visible
    if (profile.isAgencyManaged) {
      profile.profileLocked = false;
      profile.photosLocked = false;
    }
    ...
    // ✅ UPDATED: Show agency success fee to ALL viewers (so UI banner works)
    if (profile.isAgencyManaged) {
      safeProfile.isAgencyManaged = true;
      safeProfile.agencyId = profile.agencyId;
      safeProfile.agencyNameTag = profile.agencyNameTag || safeProfile.agencyNameTag;

      safeProfile.successFee = profile.successFee || 0;
      safeProfile.successFeeCurrency = profile.successFeeCurrency || 'LKR';
    }
...
export const uploadPhotos = async (req, res) => {
  try {
    ...
    const newPhotos = req.files.map((file, index) => ({
      ...
      isProfile: (profile.photos?.length || 0) === 0 && index === 0,
      uploadedAt: new Date(),
    }));
