...
export const unlockContact = async (req, res) => {
  try {
    ...
    // NOTE: default amount is just a mock/dev value
    ...
    const user = await User.findById(userId).select('contactsUnlocked isPremium subscription premiumExpiry').lean();
    ...
    // Premium users get free unlocks
    ...
    const payment = await Payment.create({
      ...
      plan: 'contact_unlock',
      status: 'succeeded',
      // ✅ FIX: Set gateway to 'mock' not default 'payhere'
      gateway: 'mock',
...
export const getUnlockedContacts = async (req, res) => {
  try {
    ...
    // Paginate the IDs first, then query only those
...
export const getContactDetails = async (req, res) => {
  try {
    ...
    const targetUser = await User.findById(targetUserId).select('email phone countryCode').lean();
    if (!targetUser) return res.status(404).json({ message: 'User not found' });
