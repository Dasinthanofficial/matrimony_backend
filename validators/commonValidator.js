...
export const updateProfileValidator = [
  ...
  body('religion').optional().trim().notEmpty().withMessage('Religion required'),
  ...
  body('city').optional().trim().notEmpty().withMessage('City required'),
  body('maritalStatus')
    .optional()
    .isIn(['never_married', 'divorced', 'widowed', 'awaiting_divorce', 'annulled'])
    .withMessage('Invalid marital status'),
...
export const reportValidator = [
  ...
  body('evidence').optional().isArray({ max: 10 }).withMessage('Evidence must be an array of up to 10 items'),
  body('evidence.*')
    .optional()
    .isString()
    ...
    .trim()
    .notEmpty()
    .withMessage('Evidence items must be non-empty strings'),
];

export const mongoIdValidator = [param('id').isMongoId().withMessage('Invalid ID format')];
