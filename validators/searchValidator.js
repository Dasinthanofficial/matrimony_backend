...
export const searchProfilesValidator = [
  ...
  query('citizenship').optional().trim().isLength({ max: 100 }).withMessage('Citizenship must be less than 100 characters'),

  query('nativeSriLankan').optional().isBoolean().withMessage('nativeSriLankan must be boolean'),

  query('sriLankanAbroad').optional().isBoolean().withMessage('sriLankanAbroad must be boolean'),

  query('excludeCountry').optional().trim().isLength({ max: 100 }).withMessage('excludeCountry too long'),
