...
export const updateUserRoleValidator = [
  param('userId').isMongoId().withMessage('Invalid user ID'),
  body('role')
    .isIn(['user', 'agency', 'admin', 'moderator', 'superadmin'])
    .withMessage('Invalid role. Must be: user, agency, admin, or moderator'),
];

// ✅ NEW: agency approval/rejection
export const approveAgencyValidator = [
  param('id').isMongoId().withMessage('Invalid agency ID'),
];

export const rejectAgencyValidator = [
  param('id').isMongoId().withMessage('Invalid agency ID'),
  body('reason').optional().trim().isLength({ max: 500 }).withMessage('Reason too long (max 500 characters)'),
];