...
import {
  ...
  approveAgencyValidator,
  rejectAgencyValidator,
} from '../validators/adminValidator.js';

import {
  getAllUsers,
  getUserDetail,
  getUserFullDetails, // ✅ NEW
  suspendUser,
  unsuspendUser,
  deleteUser,
  approveProfile,
...
import { approveAgency, rejectAgency } from '../controllers/adminAgencyController.js';
import { getAllAgencyProfiles } from '../controllers/adminAgencyProfilesController.js';
...
// ✅ NEW: SUPERADMIN ONLY full details (user + profile + agency + agencyCreatedProfiles)
router.get('/users/:userId/full', protect, superadmin, getUserFullDetails);
...
// role change (controller already enforces superadmin)
router.patch('/users/:userId/role', protect, admin, updateUserRoleValidator, handleValidation, updateUserRole);

// agency approval
router.put('/agencies/:id/approve', protect, admin, approveAgencyValidator, handleValidation, approveAgency);
router.put('/agencies/:id/reject', protect, admin, rejectAgencyValidator, handleValidation, rejectAgency);

// ✅ AGENCY PROFILES (match frontend: superadmin-only visibility)
router.get('/agency-profiles', protect, superadmin, paginationValidator, handleValidation, getAllAgencyProfiles);
